 ...
 */
const getTime = (date) => new Date(date).toTimeString().slice(0, 8);
...
function getNextFriday(date) {
  const copy = new Date(date.toJSON());
  const days = [5, 4, 3, 2, 1, 7, 6];
  const dayNow = copy.getDay();
  const newDate = copy.getDate() + days[dayNow];
 ...
 * 2, 2024 => 29
 */
const getCountDaysInMonth = (month, year) => new Date(year, month, 0).getDate();
...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  ...
  return Math.ceil((new Date(dateEnd) - new Date(dateStart)) / dayLength) + 1;
}
...
function isDateInPeriod(date, period) {
  ...
  [now, start, end] = [now, start, end].map((dateString) =>
...
function getCountWeekendsInMonth(month, year) {
  ...
  while (month - 1 === date.getMonth()) {
    ...
    date.setDate(thisDate - counter);
    counter += 1;
  }
  return days.filter((dayNumber) => dayNumber === 6 || !dayNumber).length;
}
...
function getWeekNumberByDate(date) {
  ...
  const days = (date - yearStart) / DAY_LENGTH + 1;
  return Math.ceil((days + yearStart.getDay()) / WEEK_LENGTH);
}
...
function getNextFridayThe13th(date) {
  const now = new Date(date.toJSON());
  const FRIDAY = 5;
  const BAD_DAY = 13;
  const currentMonth = now.getMonth();
  let counter = 1;
  now.setDate(BAD_DAY);
  while (now.getDay() !== FRIDAY) {
    now.setMonth(currentMonth + counter);
    counter += 1;
  }
  return now;
}
...
function getQuarter(date) {
  const quarters = [1, 2, 3, 4]
    .map((number) => `${number}`.repeat(3).split(''))
    .flat();
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  const splitter = (string) =>
    string
      ...
      .map((el) => +el)
      .reverse();
  const newDate = (args) => new Date(args[0], args[1] - 1, args[2]);
  const { start, end } = period;
  const [startDate, endDate] = [start, end].map((dateString) =>
    newDate(splitter(dateString))
  );
  const schedule = [];
  while (
    ...
    for (let i = 0; i < countWorkDays && endDate - startDate >= 0; i += 1) {
      const workDay = startDate
 ...
 * Date(2020, 2, 1) => true
 */
const isLeapYear = (date) => !(date.getFullYear() % 4);
